...
public class MessageView {
    ...
    private Message.RecipientStatus getMessageStatus(Message msg) {
        ...
        if (!msg.getSentByUserId().equalsIgnoreCase(MainChatActivity.getUserID()))
        ...
        for (int i = 0; i < MainChatActivity.getAllParticipants().size(); i++) {

            //Don't check the status of the current user
            String participant = MainChatActivity.getAllParticipants().get(i);
            if (participant.equalsIgnoreCase(MainChatActivity.getUserID()))
                continue;
